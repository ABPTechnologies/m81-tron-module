...
class TronGridClient {
     ...
     */
    async listInboundTrc20(address, opts = {}) {
        const params = new URLSearchParams({
            only_to: 'true',
            limit: String(opts.limit ?? 50),
            contract_address: opts.contract ?? trc20_1.USDT_TRC20_CONTRACT,
        });
        if (opts.minTimestampMs)
            params.set('min_timestamp', String(opts.minTimestampMs));
        const r = await this.get(`/v1/accounts/${address}/transactions/trc20?${params.toString()}`);
        return (r.data ?? []).map((t) => ({
            txID: t.transaction_id,
            from: t.from,
            to: t.to,
            value: t.value,
            contract: t.token_info?.address ?? opts.contract ?? trc20_1.USDT_TRC20_CONTRACT,
